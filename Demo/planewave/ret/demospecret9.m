%  DEMOSPECRET9 - Scattering spectra for substrate using PARFOR loop.
...
%    spectrum using the full Maxwell equations.  We show how to perform
%    parallel computing in setting up the Green function table and in the
%    loop over the photon wavelengths.
%
%  Runtime on my computer:  45 sec (much faster on cluster).
...
if ~exist( 'greentab', 'var' ) || ~greentab.ismember( layer, enei, p )
  ...
  %  precompute Green function table using PARFOR loops
  %    for a more accurate simulation of the layer the number of
  %    wavelenghts should be increased
  greentab = parset( greentab, linspace( 500, 800, 5 ), op );
end
...
%  loop over wavelengths using PARFOR loops
